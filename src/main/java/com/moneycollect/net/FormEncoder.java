...
public final class FormEncoder {
  ...
  public static String createQueryString(
    ...
    return String.join(
        "&",
        nameValueCollection.stream().filter( kv -> kv.getValue() != null)
   ...
   *
   * <p>This is a "pre-encoding" step necessary to send requests to MoneyCollect's API. Form encoding can
   * be ambiguous when it comes to nested parameters (lists or maps). MoneyCollect's API relies heavily on
  ...
  private static List<KeyValuePair<String, Object>> flattenParamsValue(
    ...
    if (value == null) {
      ...
      flatParams = singleParam(keyPrefix, JsonUtil.toJson(value).replaceAll("\"", ""));
    } else {
